...
import Myinvoices from '../../../components/User/Client/myhome/Myinvoices';
...
export class HomePage extends Component {
    ...
    showhistorycart(products) {
        //console.log(products)
        var result = null;
        if (products.length > 0) {
          result = products.map((product, index) => {
            return (
                <Myinvoices key={index} product={product}/>
            );
          });
        }
        return result;
                    ...
                    <div className="row">
                        ...
                        <div className="row-right">
                            ...
                            <table className="table table-hover">
                                ...
                                <tbody>
                                    <tr>
                                        <td className="mytable"><Link to="/admin/profile"><h2>
                                            <i className="far fa-address-card"></i> Profile</h2></Link></td>
                                    </tr>
                                    <tr>
                                        <td className="mytable"><Link to="/myinvoices"><h2>
                                            <i className="fas fa-plus-circle"></i> My Invoices</h2><span>{historycart.length}</span></Link></td>
                                    </tr>
                                    <tr>
                                        <td className="mytable"><Link to="/admin/product"><h2>
                                            <i className="fas fa-chalkboard-teacher"></i> My Product</h2><span>{myproducts}</span></Link></td>
                                    </tr>
                                    <tr>
                                        <td className="mytable"><Link to="/mycart"><h2>
                                            <i className="fas fa-cart-plus"></i> My Cart</h2><span>{cartproducts.length}</span></Link></td>
                                    </tr>
                                    <tr>
                                        <td className="mytable"><Link to="/myfavorites"><h2>
                                            <i className="fas fa-heart"></i> My Favorites</h2><span>{faproducts.length}</span></Link></td>
                                    </tr>
                                    <tr>
                                        <td className="mytable"><Link to="/myreviews"><h2>
                                            <i className="fas fa-star"></i> My Reviews</h2><span>{products.length}</span></Link></td>
                                    </tr>
                                    <tr>
